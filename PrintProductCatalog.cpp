...
// ---------------------------------------------------------------------------
__fastcall TFrPrintProductCatalog::TFrPrintProductCatalog(TComponent* Owner) : TForm(Owner)
{
	try {
		if (SetupIni.sRep8_Rep_Billings.Chk == (MainForm->CheckSumParam
			((char *)&SetupIni.sRep8_Rep_Billings.Chk,
			sizeof(SetupIni.sRep8_Rep_Billings)) - 2))
...
// ---------------------------------------------------------------------------
void __fastcall TFrPrintProductCatalog::FormClose(TObject *Sender, TCloseAction &Action)
{
	...
	SetupIni.sRep8_Rep_Billings.Chk =
		(MainForm->CheckSumParam((char *)&SetupIni.sRep8_Rep_Billings.Chk,
		sizeof(SetupIni.sRep8_Rep_Billings)) - 2);
	// Проверка контрольной сумммы блока параметров

	Action = caFree;
	flFrPrintProductCatalog = WINCLS;
}
...
void __fastcall TFrPrintProductCatalog::RzToolButton2Click(TObject *Sender) {
	...
	MainForm->SaveXlsFiles(cxGrid1, FrPrintProductCatalog->Caption, "");
}
...
void __fastcall TFrPrintProductCatalog::RzComboBox3Change(TObject *Sender) {
	...
	if (RzComboBox3->Items->Count == 0)
		return;
	if (RzComboBox3->ItemIndex < 0)
...
void __fastcall TFrPrintProductCatalog::RzToolButton1Click(TObject *Sender)
	// Кнопка обновить
{
	Timer1->Enabled = false;
	Timer1->Enabled = true;
...
void __fastcall TFrPrintProductCatalog::RzToolButton10Click(TObject *Sender) {
	cxGrid1DBBandedTableView1->OptionsView->ColumnAutoWidth =
		!cxGrid1DBBandedTableView1->OptionsView->ColumnAutoWidth;
}
// ---------------------------------------------------------------------------

void __fastcall TFrPrintProductCatalog::RzToolButton9Click(TObject *Sender) {
	cxGrid1DBBandedTableView1->OptionsView->CellAutoHeight =
		!cxGrid1DBBandedTableView1->OptionsView->CellAutoHeight;
}
...
void __fastcall TFrPrintProductCatalog::RzToolButton8Click(TObject *Sender) {
	...
	MainForm->SaveXlsFiles(cxGrid2, FrPrintProductCatalog->Caption, "");
}
...
{
	RzEdit21->Enabled = RzCheckBox21->Checked;
	if (RzEdit21->Text == RzCheckBox21->Caption &&
		RzCheckBox21->Checked == true)
		RzEdit21->Text = "";
}
// ---------------------------------------------------------------------------
...
void __fastcall TFrPrintProductCatalog::RzMenedger1ButtonClick(TObject *Sender)
{
...
{
	AnsiString SL, Su, SE;
	if (MainForm->VoidManager != true)
		Su = " where  auId =" + IntToStr(PermisHdr.KeyUserStartProgramm);
	else
		Su = "";
	SL = " Declare @IdUser int; SET @IdUser=:D0; "
		 " delete from clt_spOtraslCheck where IdIns=@IdUser and IdL24Table=4 "
		 " insert into clt_spOtraslCheck  (IdIns,IdCheck,IdL24Table) select DISTINCT @IdUser , auId,4 from dbo.aa_Users "
		+ Su;
	Qw->Close();
	Qw->SQL->Clear();
	Qw->SQL->Add(SL);
...
// ---------------------------------------------------------------------------
void __fastcall TFrPrintProductCatalog::List()
{
	int i;
	AnsiString vsSQLQw;
	/*
	 ...
	 */
	vsSQLQw = " select DeptID,DeptName,Fl from dbo.Depts where Fl is not null and DelData is null order by Fl ";
	Qw->Close();
	Qw->SQL->Clear();
	Qw->SQL->Add(vsSQLQw);
	Qw->Open();
	...
	while (!Qw->Eof)
		  {
		   PLFrom->Properties->Items->AddCheckItem(Qw->Fields->Fields[1]->AsString, Qw->Fields->Fields[1]->AsString);
...
void __fastcall TFrPrintProductCatalog::RzRadioButton2Click(TObject *Sender)
{
  tmpSqlDDel = 0;
}
//---------------------------------------------------------------------------

void __fastcall TFrPrintProductCatalog::RzRadioButton1Click(TObject *Sender)
{
  tmpSqlDDel = 1;
}
//---------------------------------------------------------------------------
